...
namespace DungeonGenerator
{
    public class SecretRoom : ProceduralRoom
    {
        public override bool CanCreate(int x, int y)
        {
            ...
            {
                RoomBehaviour room = DungeonManager.Dungeon.GetRoom(x, y + 1);
            ...
            {
                RoomBehaviour room = DungeonManager.Dungeon.GetRoom(x - 1, y);
                return CheckSecretRoom(room);
            }
        ...
        private bool CheckSecretRoom(RoomBehaviour room)
        {
            if (room != null)
            {
